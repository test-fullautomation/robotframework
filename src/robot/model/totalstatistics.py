...
class TotalStatistics:
    ...
    def message(self) -> str:
        ...
        template = '%d %s%s, %d passed, %d failed, %d unknown' 
        if skipped:
            return ((template + ', %d skipped')
                    % (total, test_or_task, end, passed, failed, unknown, skipped))
        return template % (total, test_or_task, end, passed, failed, unknown)
