...
Verify total stats
    ...
    Verify stat    ${all[0]}    label:All Tests    pass:12    fail:1    skip:0    unknown:0

Verify tag stats
    [Arguments]    ${file}
    ${stats} =    Get Tag Stats    ${OUTDIR}${/}${file}
    Length Should Be    ${stats}    4
    Verify stat    ${stats[0]}    label:f1 AND t1    pass:5    fail:1    skip:0    unknown:0
    ...    info:combined    combined:f1 AND t1
    Verify stat    ${stats[1]}    label:d1    pass:1    fail:0    skip:0    unknown:0
    Verify stat    ${stats[2]}    label:t1    pass:5    fail:1    skip:0    unknown:0
    ...    links:T1:http://t/1    doc:the doc
    Verify stat    ${stats[3]}    label:t2    pass:2    fail:0    skip:0    unknown:0
    ...    links:T2:http://t/2
