*** Test Cases ***
Invalid condition
    [Documentation]    UNKNOWN    Invalid IF condition: \
...
Invalid condition with other error
    [Documentation]    UNKNOWN     ELSE branch cannot be empty.
    IF    bad    Not run    ELSE
...
IF followed by ELSE IF
    [Documentation]    UNKNOWN    STARTS: Invalid IF condition: Evaluating expression 'ELSE IF' failed:
...
Empty ELSE IF 2
    [Documentation]    UNKNOWN    Invalid ELSE IF condition: \
    ...    Evaluating expression 'ELSE' failed: NameError: name 'ELSE' is not defined nor importable as module
    IF    False    Not run    ELSE IF    ELSE    Not run

ELSE IF without branch 1
    [Documentation]    UNKNOWN    ELSE IF branch cannot be empty.
...
ELSE IF after ELSE 1
    [Documentation]    UNKNOWN    ELSE IF not allowed after ELSE.
...
Multiple ELSEs 1
    [Documentation]    UNKNOWN    Only one ELSE allowed.
...
Nested IF 1
    [Documentation]    UNKNOWN    Inline IF cannot be nested.
...
Unnecessary END
    [Documentation]    UNKNOWN    Keyword 'BuiltIn.No Operation' expected 0 arguments, got 1.
    IF    True     No operation    ELSE    Log    END
    IF    False    Not run         ELSE    No operation    END

Invalid END after inline header
    [Documentation]    UNKNOWN    END is not allowed in this context.
    IF    True    Log    Executed inside inline IF
        Log   Executed outside IF
    END

Assign in IF branch
    [Documentation]    UNKNOWN    Inline IF branches cannot contain assignments.
    IF    False    ${x} =    Whatever
...
Invalid number of variables in assign
    [Documentation]    UNKNOWN    Cannot set variables: Expected 2 return values, got 3.
    ${x}    ${y} =    IF    False    Create list    x    y     ELSE    Create list    x    y    z

Invalid value for list assign
    [Documentation]    UNKNOWN    Cannot set variable '\@{x}': Expected list-like value, got string.
    @{x} =    IF    True    Set variable    String is not list

Invalid value for dict assign
    [Documentation]    UNKNOWN    Cannot set variable '\&{x}': Expected dictionary-like value, got string.
...
Control structures are not allowed with assignment
    [Documentation]    UNKNOWN    Inline IF with assignment can only contain keyword calls.
    Assign with RETURN
